              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse bg-white dark:bg-gray-800 shadow rounded-xl overflow-hidden">
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                    {items.map((row, idx) => (
                      <tr key={idx}>
                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{row.user?.name || 'Unknown'}</td>
                        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{row.user?.email || '-'}</td>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {items.map((row, idx) => (
                  <div key={idx} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-300 mb-3">{row.user?.email || '-'}{row.user?.phone ? ` • ${row.user.phone}` : ''}</div>
