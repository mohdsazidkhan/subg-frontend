...
export default function Navbar() {
  ...
  const studentLinks = (
    <>
      <Link
        ...
        to="/subscription"
        className={`px-4 py-2 rounded-xl font-semibold flex items-center gap-2 shadow-md transition-all duration-200 text-white text-sm bg-gradient-to-r ${hasActiveSubscription() ? 'from-green-500 to-green-700 hover:from-green-600 hover:to-green-800' : 'from-orange-500 to-orange-700 hover:from-orange-600 hover:to-orange-800'} scale-95 hover:scale-100`}
      >
        <span className="text-lg flex items-center">
          {hasActiveSubscription() ? <FaCalendarAlt /> : <FaCreditCard />}
        </span>
        <span className="hidden sm:inline">{hasActiveSubscription() ? 'Active' : 'Subscribe'}</span>
      </Link>
      <Link
        title="My Profile"
        to="/profile"
  ...
  const Hamburger = (
    <button
      className="md:hidden flex items-center justify-center p-2 rounded-full bg-white/70 dark:bg-gray-800/70 shadow-lg border border-gray-200 dark:border-gray-700 hover:bg-white/90 dark:hover:bg-gray-700 transition-all"
  ...
  return (
    <nav className="backdrop-blur-xl bg-gradient-to-r from-blue-50/80 via-purple-50/80 to-indigo-50/80 dark:from-gray-900/80 dark:via-blue-900/80 dark:to-purple-900/80 border-b border-white/30 dark:border-gray-800 shadow-xl fixed top-0 left-0 w-full z-50 transition-all">
      <div className="container mx-auto px-4 sm:px-8 flex justify-between items-center h-16 relative">
        {/* Logo */}
        <Link
          to={user && isAdmin() && hasAdminPrivileges() ? "/admin/dashboard" : "/"}
          className="flex items-center gap-2 group"
        >
          <img src={logoimage} alt="SUBG QUIZ" title="SUBG QUIZ" className="logo" />
        </Link>
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          >
            {theme === 'dark' ? <MdLightMode className="text-white text-xl" /> : <MdDarkMode className="text-yellow-700 text-xl" />}
          </button>
          {user ? (
            <>
            ...
            {user ? (
              <>
                ...
                {isAdmin() && hasAdminPrivileges() && (
                  <button
                    title="Toggle Admin Menu"
                    onClick={() => { setMobileMenu(false); dispatch(toggleSidebar()); }}
                    className="px-4 py-2 rounded-xl font-semibold flex items-center gap-2 shadow-md transition-all duration-200 text-white text-sm bg-gradient-to-r from-gray-600 to-gray-800 hover:from-gray-700 hover:to-gray-900 scale-95 hover:scale-100"
                  >
                    {isOpen ? <MdClose /> : <MdMenu />}
                    <span className="hidden sm:inline">Menu</span>
