...
const ViewToggle = ({ currentView, onViewChange, views = ['table', 'list', 'grid'] }) => {
  ...
  return (
    <div className="flex items-center justify-evenly space-x-1 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-1 shadow-sm">
      {views.map((view) => {
        ...
        return (
          <button
            ...
            className={`flex items-center space-x-1 sm:space-x-2 px-2 py-1 sm:px-3 sm:py-2 md:px-4 md:py-1 rounded-md text-xs sm:text-sm md:text-base font-medium transition-all duration-200 ${
              isActive
